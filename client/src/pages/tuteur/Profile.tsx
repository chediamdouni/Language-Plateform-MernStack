...
const TuteurProfile = () => {
          ...
          <tbody>
            {getUniqueTimes(availability).map((time) => (
              <tr key={time}>
                ...
                {days.map((day) => (
                  <td className="px-2 py-4" key={`${day}-${time}`}>
                    {/* Vérifiez si le jour a des créneaux horaires */}
                    {slotsByDay[day] &&
                      slotsByDay[day].find(
                        (slot) => slot.startTime === time
                      ) && (
                        <span className="underline">
                          {/* Affichez l'heure de fin si le créneau est trouvé */}
                          {/* {slotsByDay[day]
                          ?.find((slot) => slot.startTime === time)
                          ?.endTime || undefined} */}
                          {time}
                        </span>
                      )}
  ...
  const renderTimeSlots = (formattedAvailability: {
    [day: string]: AvailabilitySlot[];
  }): React.ReactElement[] => {
    const times = getUniqueTimes(availability);
        ...
        {days.map((day) => (
          <td key={`${day}-${time}`}>
            {formattedAvailability[day] &&
              formattedAvailability[day].find(
                (slot) => slot.startTime === time
              ) && (
                <span>
                  {formattedAvailability[day]?.find(
  ...
  const getUniqueTimes = (availability: AvailabilitySlot[]): string[] => {
    ...
    return Array.from(timeSet).sort(); // Unique sorted time slots
  };
  // useEffect(() => {
  ...
  //       setAvailability(response.data.availability);
  //     } catch (error) {
  //       console.log("Error fetching avaialability ", error);
  //     }
  //   };
  //   fetchAvailability();
  // }, []);
  return (
    <TuteurLayout>
      <div
        className="flex justify-center mt-6 p-6 font-korto font-sans"
