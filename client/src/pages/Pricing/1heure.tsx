...
const OneHour: React.FC<OnehourProps> = ({ onValuesChange }) => {
  const [selectedPlan, setSelectedPlan] = useState<string>("monthly");
  const [totalPrice, setTotalPrice] = useState(0);
  const navigate = useNavigate();

  const plans = [
    { id: "monthly", name: "Mensuel", price: 79, period: "/ mois" },
    {
      id: "quarter",
      name: "Trimestriel",
      price: 71,
  ...
  const calculatePrice = (selectedValue: string) => {
    ...
    switch (selectedValue) {
      case "monthly":
        TotalpricePerMonth = 79 * 12;
        setTotalPrice(TotalpricePerMonth);
        break;
      case "quarter":
        TotalpricePerQuarter = Math.round(71 * 12 * 0.9);
        setTotalPrice(TotalpricePerQuarter);
        break;
      case "annual":
        TotalpricePerYear = Math.round(59 * 12 * 0.75);
  ...
  return (
    <div className="bg-white rounded-xl shadow-md overflow-hidden">
      <div className="bg-indigo-600 p-4 text-white">
        <h3 className="text-xl font-semibold">1 heure par semaine</h3>
