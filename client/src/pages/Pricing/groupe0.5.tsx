...
import { FaCheck, FaArrowRight, FaUsers } from "react-icons/fa";
import { useNavigate } from "react-router-dom";

interface HalfGroupeProps {
  onValuesChange: (monthly: number, quarter: number, annual: number) => void;
}

const HalfGroupe: React.FC<HalfGroupeProps> = ({ onValuesChange }) => {
  const [selectedPlan, setSelectedPlan] = useState<string>("monthly");
  const [totalPrice, setTotalPrice] = useState(0);
  const navigate = useNavigate();

  const plans = [
    { id: "monthly", name: "Mensuel", price: 21, period: "/ mois" },
    {
      id: "quarter",
      name: "Trimestriel",
      price: 19,
  ...
  const calculatePrice = (selectedValue: string) => {
    ...
    switch (selectedValue) {
      case "monthly":
        TotalpricePerMonth = 21 * 12;
        setTotalPrice(TotalpricePerMonth);
        break;
      case "quarter":
        TotalpricePerQuarter = Math.round(19 * 12 * 0.9);
        setTotalPrice(TotalpricePerQuarter);
        break;
      case "annual":
        TotalpricePerYear = Math.round(16 * 12 * 0.75);
    ...
    <div className="bg-white rounded-xl shadow-md overflow-hidden">
      <div className="bg-indigo-600 p-4 text-white">
        <h3 className="text-xl font-semibold">
          Cours en groupe - 30 minutes par semaine
        </h3>
      ...
      <div className="bg-gray-50 p-4 border-t border-gray-200">
        <ul className="space-y-2">
          ...
          <li className="flex items-center text-sm text-gray-600">
            <FaCheck className="text-green-500 mr-2" /> Interaction avec
            d'autres apprenants
          </li>
          <li className="flex items-center text-sm text-gray-600">
